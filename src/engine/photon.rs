...
lazy_static! {
    static ref WATERMARK: PhotonImage = {
        ...
        let watemark = open_image_from_bytes(data).unwrap();
        transform::resize(&watemark, 64, 64, transform::SamplingFilter::Nearest)
    };
}
    ...
    fn transform(&mut self, op: &Resize) {
        let img = match resize::ResizeType::from_i32(op.rtype).unwrap() {
            resize::ResizeType::Normal => transform::resize(
                ...
                op.width,
                op.height,
                resize::FilterType::from_i32(op.filter).unwrap().into(),
            ),
